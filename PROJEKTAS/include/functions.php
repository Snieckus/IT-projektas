...
function checkname ($username){   // Vartotojo vardo sintakse
   ...
   function checkvardas ($vardas){   // Vartotojo vardo sintakse
	if (!$vardas || strlen($vardas = trim($vardas)) == 0) 
		 {$_SESSION['vardas_error']=
			  "<font size=\"2\" color=\"#ff0000\">* Neįvestas vartotojo vardas</font>";
		  "";
		  return false;}
		 elseif (!preg_match("/^([a-zA-Z])*$/", $vardas))  /* Check if username is not alphanumeric */ 
		 {$_SESSION['vardas_error']=
			 "<font size=\"2\" color=\"#ff0000\">* Jūsų vardas gali būti sudarytas<br>
			 &nbsp;&nbsp;tik iš raidžių</font>";
		  return false;}
		 else return true;
}
 ...
 function checkdb($username) {  // iesko DB pagal varda, grazina {vardas,slaptazodis,lygis,id} ir nustato name_error
	     ...
	     $uname = $upass = $ulevel = $uid = $umail = $uvardas =  null;
     ...
     return array($uname,$upass,$ulevel,$uid,$umail,$uvardas);
 }
